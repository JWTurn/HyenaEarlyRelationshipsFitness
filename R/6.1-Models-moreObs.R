...
DT.50 <- DT.25[nSession.obs>=50]

DT.25[!is.na(annual_rs),uniqueN(ego), period]
DT.50[!is.na(annual_rs),uniqueN(ego), period]


DT.25[!is.na(longevity_years),uniqueN(ego), period]
DT.50[!is.na(longevity_years),uniqueN(ego), period]
...
# DT.50.lcd.7.pless <- DT.50.lcd.7.p[, sum(estimate < estimate.obs), term]
# DT.50.lcd.7.pless[,'pless'] <- DT.50.lcd.7.pless$V1/1000
# DT.50.lcd.7.pmore <- DT.50.lcd.7.p[, sum(estimate > estimate.obs), term]
# DT.50.lcd.7.pmore[,'pmore'] <- DT.50.lcd.7.pmore$V1/1000
...
DT.50.lcd.2.p <-
	merge(
		DT.50.lcd.2[iteration != 0],
		DT.50.lcd.2[iteration == 0, .(effect, group, term, estimate)],
		by = c('effect', 'group', 'term'),
		suffixes = c('', '.obs')
	)

# DT.50.lcd.2.pless <- DT.50.lcd.2.p[, sum(estimate < estimate.obs), term]
# DT.50.lcd.2.pless[,'pless'] <- DT.50.lcd.2.pless$V1/1000
# DT.50.lcd.2.pmore <- DT.50.lcd.2.p[, sum(estimate > estimate.obs), term]
# DT.50.lcd.2.pmore[,'pmore'] <- DT.50.lcd.2.pmore$V1/1000


DT.50.lcd.2.pboth <- DT.50.lcd.2.p[, .(estimate = unique(estimate.obs), min = min(estimate), max = max(estimate),
																			 sumless = sum(estimate < estimate.obs), summore = sum(estimate > estimate.obs),
																			 pless = sum(estimate < estimate.obs)/1000, pmore = sum(estimate > estimate.obs)/1000),
																	 term]
...
#### best model for longevity DI ####
# all and directed only
DT.50.lpg.7 <- DT.50[period == 'postgrad', {
	m <- glmer(
		log(longevity_years) ~
			ego_period_rank +
			scale(twi_degree) +
...
DT.50.lpg.7.p <-
	merge(
		DT.50.lpg.7[iteration != 0],
		DT.50.lpg.7[iteration == 0, .(effect, group, term, estimate)],
		by = c('effect', 'group', 'term'),
		suffixes = c('', '.obs')
	)

# DT.50.lpg.7.pless <- DT.50.lpg.7.p[, sum(estimate < estimate.obs), term]
# DT.50.lpg.7.pless[,'pless'] <- DT.50.lpg.7.pless$V1/1000
# DT.50.lpg.7.pmore <- DT.50.lpg.7.p[, sum(estimate > estimate.obs), term]
# DT.50.lpg.7.pmore[,'pmore'] <- DT.50.lpg.7.pmore$V1/1000


DT.50.lpg.7.pboth <- DT.50.lpg.7.p[, .(estimate = unique(estimate.obs), min = min(estimate), max = max(estimate),
																			 sumless = sum(estimate < estimate.obs), summore = sum(estimate > estimate.obs),
																			 pless = sum(estimate < estimate.obs)/1000, pmore = sum(estimate > estimate.obs)/1000),
																	 term]
...
#### best model for longevity Adult ####
# all and directed only
DT.50.lad.7 <- DT.50[period == 'adult', {
	m <- glmer(
		log(longevity_years) ~
			ego_period_rank +
			scale(twi_degree) +
...
# DT.50.lad.7.pless <- DT.50.lad.7.p[, sum(estimate < estimate.obs), term]
# DT.50.lad.7.pless[,'pless'] <- DT.50.lad.7.pless$V1/1000
# DT.50.lad.7.pmore <- DT.50.lad.7.p[, sum(estimate > estimate.obs), term]
# DT.50.lad.7.pmore[,'pmore'] <- DT.50.lad.7.pmore$V1/1000
...
# DT.50.acd.7.pless <- DT.50.acd.7.p[, sum(estimate < estimate.obs), term]
# DT.50.acd.7.pless[,'pless'] <- DT.50.acd.7.pless$V1/1000
# DT.50.acd.7.pmore <- DT.50.acd.7.p[, sum(estimate > estimate.obs), term]
# DT.50.acd.7.pmore[,'pmore'] <- DT.50.acd.7.pmore$V1/1000
...
DT.50.acd.2.p <-
	merge(
		DT.50.acd.2[iteration != 0],
		DT.50.acd.2[iteration == 0, .(effect, group, term, estimate)],
		by = c('effect', 'group', 'term'),
		suffixes = c('', '.obs')
	)

# DT.50.acd.2.pless <- DT.50.acd.2.p[, sum(estimate < estimate.obs), term]
# DT.50.acd.2.pless[,'pless'] <- DT.50.acd.2.pless$V1/1000
# DT.50.acd.2.pmore <- DT.50.acd.2.p[, sum(estimate > estimate.obs), term]
# DT.50.acd.2.pmore[,'pmore'] <- DT.50.acd.2.pmore$V1/1000


DT.50.acd.2.pboth <- DT.50.acd.2.p[, .(estimate = unique(estimate.obs), min = min(estimate), max = max(estimate),
																			 sumless = sum(estimate < estimate.obs), summore = sum(estimate > estimate.obs),
																			 pless = sum(estimate < estimate.obs)/1000, pmore = sum(estimate > estimate.obs)/1000),
																	 term]
...
DT.50.apg.19.a <- DT.50[period == 'postgrad', {
	m <- glmer(
		log(annual_rs) ~
			ego_period_rank +
			scale(twi_strength) +
...
DT.50.apg.19.a.p <-
	merge(
		DT.50.apg.19.a[iteration != 0],
		DT.50.apg.19.a[iteration == 0, .(effect, group, term, estimate)],
		by = c('effect', 'group', 'term'),
		suffixes = c('', '.obs')
	)

# DT.50.apg.19.a.pless <- DT.50.apg.19.a.p[, sum(estimate < estimate.obs), term]
# DT.50.apg.19.a.pless[,'pless'] <- DT.50.apg.19.a.pless$V1/1000
# DT.50.apg.19.a.pmore <- DT.50.apg.19.a.p[, sum(estimate > estimate.obs), term]
# DT.50.apg.19.a.pmore[,'pmore'] <- DT.50.apg.19.a.pmore$V1/1000

DT.50.apg.19.a.pboth <- DT.50.apg.19.a.p[, .(estimate = unique(estimate.obs), min = min(estimate), max = max(estimate),
																						 sumless = sum(estimate < estimate.obs), summore = sum(estimate > estimate.obs),
																						 pless = sum(estimate < estimate.obs)/1000, pmore = sum(estimate > estimate.obs)/1000),
																				 term]
...
# directed only
DT.50.apg.7 <- DT.50[period == 'postgrad', {
	m <- glmer(
		log(annual_rs) ~
			ego_period_rank +
			scale(twi_degree) +
...
DT.50.apg.7.p <-
	merge(
		DT.50.apg.7[iteration != 0],
		DT.50.apg.7[iteration == 0, .(effect, group, term, estimate)],
		by = c('effect', 'group', 'term'),
		suffixes = c('', '.obs')
	)

# DT.50.apg.7.pless <- DT.50.apg.7.p[, sum(estimate < estimate.obs), term]
# DT.50.apg.7.pless[,'pless'] <- DT.50.apg.7.pless$V1/1000
# DT.50.apg.7.pmore <- DT.50.apg.7.p[, sum(estimate > estimate.obs), term]
# DT.50.apg.7.pmore[,'pmore'] <- DT.50.apg.7.pmore$V1/1000


DT.50.apg.7.pboth <- DT.50.apg.7.p[, .(estimate = unique(estimate.obs), min = min(estimate), max = max(estimate),
																			 sumless = sum(estimate < estimate.obs), summore = sum(estimate > estimate.obs),
																			 pless = sum(estimate < estimate.obs)/1000, pmore = sum(estimate > estimate.obs)/1000),
																	 term]
...
#### best models for ARS Adult ####
# directed only
DT.50.aad.7 <- DT.50[period == 'adult', {
	m <- glmer(
		log(annual_rs) ~
			ego_period_rank +
			scale(twi_degree) +
...
# DT.50.aad.7.pless <- DT.50.aad.7.p[, sum(estimate < estimate.obs), term]
# DT.50.aad.7.pless[,'pless'] <- DT.50.aad.7.pless$V1/1000
# DT.50.aad.7.pmore <- DT.50.aad.7.p[, sum(estimate > estimate.obs), term]
# DT.50.aad.7.pmore[,'pmore'] <- DT.50.aad.7.pmore$V1/1000
